...
export async function translateTreeBestEffort(
  treeInput: CompositionTree,
  supabase: SupabaseClient<Database>,
): Promise<TranslateResult> {
  const key = process.env.LOVABLE_API_KEY;
  if (!key) return { tree: treeInput, translated: 0, skipped: true, reason: "no-key" };
  ...
  if (jobs.length === 0) {
    return { tree: treeInput, translated: 0, skipped: true, reason: "up-to-date" };
  }
  ...
  if (!match) return { tree: treeInput, translated: 0, skipped: true, reason: "no-json" };
  let parsed: {
  ...
  try {
    ...
    return { tree: treeInput, translated: 0, skipped: true, reason: "parse-error" };
  }
  ...
  for (const r of parsed.results ?? []) {
    ...
    // Sella el valor base para no re-traducir en el próximo save.
...
export const translateCompositionTree = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<TranslateResult> => {
    return translateTreeBestEffort(data.tree, context.supabase);
  });