...
export function emitConciergeCase(ctx: SubMotorContext): ConciergeOutcome {
    ...
    makeEvent(
      ...
      cursor,
      {
        decision: {
          ...
          recommendation_id: caseId,
          rationale: isSlow
            ? "Primera respuesta lenta (SLA superado)"
            : "Primera respuesta dentro de SLA",
          accepted: true,
        },
      },
      {
  ...
  if (prng.bool(abandonBeforeProposal)) {
    ...
    return {
      events,
      cursor_ms: cursor + 30 * MINUTE_MS,
      case_id: caseId,
      proposal_id: null,
      status: "abandoned",
      sla_response_ms: firstResponseGap,
      slow_response: isSlow,
    };
  }
