...
export function pickTerritorialPath(
  ...
  const candidates: Array<[DestinationSlug, number]> = ORIENTE_MAYA_NODES.filter(
    (n) => n !== primary,
  ).map((n) => {
...
function weightedIndex(entries: ReadonlyArray<[unknown, number]>, prng: Prng): number {
