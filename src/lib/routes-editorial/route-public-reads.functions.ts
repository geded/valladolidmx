...
import type { EditorialRouteCardDTO, EditorialRouteDetailDTO } from "./route-public-contract";

const SLUG = /^[a-z0-9-]{1,120}$/;

export const listPublicRoutes = createServerFn({ method: "GET" })
  .inputValidator((input?: { destino?: string | null; limit?: number }) => ({
    destino: typeof input?.destino === "string" && SLUG.test(input.destino) ? input.destino : null,
