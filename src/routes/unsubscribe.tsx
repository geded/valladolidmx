...
function UnsubscribePage() {
  ...
  return (
    <main className="mx-auto flex min-h-[70vh] max-w-md flex-col items-center justify-center gap-6 px-6 py-16 text-center">
      <div className="text-xs font-bold uppercase tracking-[0.2em] text-primary">Valladolid.mx</div>
      {state.kind === "loading" && <p>Validando enlace…</p>}
      {state.kind === "invalid" && (
        <>
          <h1 className="text-2xl font-semibold">Enlace no válido</h1>
          <p className="text-muted-foreground">
            El enlace expiró o ya fue usado. Puedes gestionar tus preferencias desde tu cuenta.
          </p>
        </>
      )}
      {state.kind === "already" && (
        <>
          ...
          <p className="text-muted-foreground">No recibirás más correos de esta lista.</p>
        </>
      )}
      {state.kind === "ready" && (
        <>
          <h1 className="text-2xl font-semibold">¿Cancelar los correos a {state.email}?</h1>
          <p className="text-muted-foreground">Dejarás de recibir notificaciones de esta lista.</p>
      ...
      {state.kind === "success" && (
        <>
          ...
          <p className="text-muted-foreground">Listo. Puedes cerrar esta ventana.</p>
        </>
      )}
      {state.kind === "error" && (
        <>
