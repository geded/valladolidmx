          ...
          <section className="grid gap-6 overflow-hidden rounded-[2rem] border border-border bg-card p-5 shadow-elevated sm:p-7 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:p-9">
            <div className="order-2 lg:order-1">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-pill border border-primary/30 bg-primary/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.16em] text-primary">Casa completa</span>
                <span className="rounded-pill border border-border bg-background/70 px-3 py-1 text-[11px] uppercase tracking-[0.16em] text-muted-foreground">Valladolid · Oriente Maya</span>
              </div>
              <h1 className="mt-4 font-serif text-4xl leading-[1.05] tracking-tight text-foreground sm:text-5xl">Casa de patio colonial</h1>
              <p className="mt-3 text-lg text-foreground/80">Una estancia privada para vivir Valladolid con calma y descubrir el Oriente Maya.</p>
              <dl className="mt-6 grid grid-cols-2 gap-x-5 gap-y-4 text-sm">
                {facts.slice(0, 4).map(({ icon: Icon, label, value }) => <div key={label} className="flex items-start gap-2"><Icon className="mt-0.5 size-4 shrink-0 text-primary" /><div><dt className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{label}</dt><dd className="text-foreground/90">{value}</dd></div></div>)}
              </dl>
              <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                <Button size="lg" className="w-full rounded-pill px-7 sm:w-auto">Consultar disponibilidad <ArrowRight className="ml-2 size-4" /></Button>
                <Button size="lg" variant="outline" className="w-full rounded-pill px-7 sm:w-auto">Agregar a Mi Viaje</Button>
              </div>
            </div>
            <div className="order-1 grid grid-cols-2 gap-3 sm:gap-4 lg:order-2">
              <img src={MEDIA.hero} alt="Casa colonial con patio y alberca en Valladolid" className="col-span-2 h-56 w-full rounded-3xl object-cover shadow-elevated sm:h-72 lg:h-80" />
              <img src={MEDIA.patio} alt="Patio colonial" className="hidden h-36 w-full rounded-2xl object-cover shadow-soft sm:block sm:h-44" />
              <img src={MEDIA.campo} alt="Territorio cercano a Valladolid" className="hidden h-36 w-full rounded-2xl object-cover shadow-soft sm:block sm:h-44" />
            </div>
          </section>
        </Container>
