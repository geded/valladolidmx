  ...
  return (
    ...
    >
      ...
      <main>
          ...
          <div className="mx-auto grid min-h-[inherit] max-w-[1500px] items-center px-6 py-12 sm:px-10 lg:px-16">
            <div className="max-w-2xl">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h1 className="font-serif text-6xl leading-[0.9] sm:text-7xl lg:text-8xl">Zazil Tunich</h1>
        ...
        <div className="mx-auto grid max-w-[1500px] gap-10 px-6 py-10 sm:px-10 lg:grid-cols-[0.9fr_1.25fr_0.95fr_0.9fr] lg:px-16">
          <Story />
          <FeaturedExperience />
          <VisitInfo />
          <TerritorialContext />
...
function SectionTitle({ children }: { children: ReactNode }) {
  return <><h2 className="font-serif text-3xl">{children}</h2><div className="mt-3 h-px w-10 bg-[#c78a14]" /></>;
}

function Story() {
  return <section><SectionTitle>Por qué es extraordinario</SectionTitle><p className="mt-5 text-sm leading-6 opacity-80">Zazil Tunich es un cenote-museo que revela la profundidad del vínculo maya con el agua y el inframundo. Formaciones milenarias, luz que esculpe la piedra y relatos ancestrales se entretejen en una experiencia que inspira respeto y asombro.</p><div className="mt-6 grid grid-cols-2 gap-px bg-current/15 text-xs"><MiniFact icon={<Sparkles />} text="Recorridos guiados por intérpretes locales" /><MiniFact icon={<UtensilsCrossed />} text="Cocina de la región con identidad maya" /><MiniFact icon={<Leaf />} text="Nado en cenote" /><MiniFact icon={<Accessibility />} text="Información sobre accesibilidad" /></div></section>;
}

function MiniFact({ icon, text }: { icon: ReactNode; text: string }) {
  return <div className="flex min-h-24 items-center gap-3 bg-inherit p-3"><span className="text-[#c78a14] [&>svg]:size-6">{icon}</span><span>{text}</span></div>;
}

function FeaturedExperience() {
  return <section id="experiencias"><SectionTitle>Experiencias destacadas</SectionTitle><div className="relative mt-5 aspect-[4/3] overflow-hidden rounded-2xl border border-[#c78a14]/50"><img src={HERO} alt="Ambiente subterráneo de Zazil Tunich" className="size-full object-cover" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/80 to-transparent p-6 text-white"><h3 className="font-serif text-3xl">Cena romántica subterránea</h3><p className="mt-2 text-sm text-white/75">Una velada íntima en un entorno natural único, con sabores locales y atmósfera ancestral.</p><div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-white/10 px-3 py-1">Cena</span><span className="rounded-full bg-white/10 px-3 py-1">Ambiente íntimo</span><span className="rounded-full bg-white/10 px-3 py-1">Entorno natural</span></div></div></div></section>;
}
...
function TerritorialContext() {
  return <section><SectionTitle>Contexto territorial</SectionTitle><div className="mt-5 rounded-2xl border border-[#c78a14]/50 bg-[#143c2e] p-6 text-white"><p className="text-xs uppercase tracking-[0.2em] text-white/75">Valladolid</p><div className="my-5 border-l border-dashed border-amber-300 pl-5"><span className="rounded bg-black/35 px-2 py-1 text-xs">6 km</span><p className="mt-16 font-medium">Zazil Tunich</p><p className="text-xs text-white/70">Cenote-Museo</p></div></div><p className="mt-4 text-sm leading-6 opacity-75">Ubicado en el Oriente Maya de Yucatán, donde la naturaleza, la historia y las tradiciones vivas invitan a explorar más.</p></section>;
}
