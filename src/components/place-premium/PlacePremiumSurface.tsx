      ...
      <div className="flex flex-col justify-center rounded-3xl border border-border bg-card p-6 sm:p-8">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          {content.hero.supporting.map((media) => (
            <DemoImage key={media.url} media={media} className="aspect-[3/2] rounded-2xl" />
          ))}
        </div>
      </div>
    </section>
  ...
  return (
    <section aria-labelledby="galeria-lugar-title">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        {rest.map((media) => (
          <DemoImage key={media.url} media={media} className="aspect-[4/3] rounded-3xl" />
        ))}
      </div>
    </section>
  );
    ...
    <section aria-labelledby="galeria-lugar-title">
      ...
      <ul className="mt-6 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2">
        {content.gallery.items.map((media) => (
          <li key={media.url} className="w-[78%] shrink-0 snap-start sm:w-[46%] lg:w-[31%]">
