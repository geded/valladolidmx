...
function withGovernedMedia(item: TourismCardVM, index: number): TourismCardVM {
  ...
  return {
    ...
    badges: [...item.badges, { label: "Visual conceptual · reemplazo pendiente", tone: "warning" }],
    rationale:
      item.rationale ??
      "Imagen conceptual generada con IA para validar la composición; no documenta este establecimiento.",
  };
}
...
export function RestaurantsPremiumListingSurface({
  ...
  const items = useMemo(() => dto.items.map(withGovernedMedia), [dto.items]);
