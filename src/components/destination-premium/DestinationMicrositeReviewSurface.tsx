...
export function DestinationMicrositeReviewSurface() {
  return (
    ...
    >
      ...
      <Container className="mt-8">
        <section className="rounded-[1.5rem] border border-black/10 bg-card/80 p-5 shadow-sm">
          <SectionHeading
        ...
        <div className="max-w-[43rem]">
          ...
          <div className="mt-5 flex flex-wrap gap-2 lg:mt-7 lg:gap-3">
            ...
            <Button
              size="lg"
              variant="outline"
              className="rounded-full border-white/40 bg-card/10 px-6 text-white backdrop-blur hover:bg-card/20 hover:text-white"
            >
              Armar mi viaje
            </Button>
          </div>
...
function TerritoryContext() {
  return (
    <nav
      ...
      className="flex min-h-11 items-center gap-1.5 overflow-x-auto whitespace-nowrap rounded-full border border-black/10 bg-card px-3 text-xs shadow-sm sm:min-h-12 sm:gap-2 sm:px-5 sm:text-sm"
    >
      <a
        href="/"
        aria-label="Volver al inicio"
        title="Inicio"
        className="grid size-8 shrink-0 place-items-center rounded-full text-[var(--selva)] transition-colors hover:bg-[var(--brand-surface-soft)]"
      >
        <Home className="size-4" aria-hidden />
      </a>
      <span className="text-black/30">/</span>
      <a href="/oriente-maya" className="font-medium text-[var(--selva)]">
        Oriente Maya
      </a>
      <span className="text-black/30">/</span>
...
function DestinationIntro() {
  return (
    <section className="grid gap-4 rounded-[1.5rem] border border-black/10 bg-card p-5 shadow-sm sm:p-7 lg:grid-cols-[.95fr_1.05fr] lg:gap-7 lg:rounded-[2rem] lg:p-10">
      <div>
        ...
        <div className="mt-2 flex gap-2 overflow-x-auto pb-1 lg:mt-3 lg:flex-wrap lg:overflow-visible">
          {["Estoy planeando", "Ya estoy aquí", "Viajo en pareja", "Viajo en familia"].map(
            (item) => (
              <button
                key={item}
                className="shrink-0 rounded-full border border-white/25 px-3 py-1.5 text-[11px] text-white/85 hover:bg-card/10 lg:px-4 lg:py-2 lg:text-xs"
              >
                {item}
              </button>
            ),
...
function SectionHeading({
  ...
  return (
    <header className={compact ? "mb-4" : "mb-5"}>
      <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[var(--brand-primary-strong)]">
        {eyebrow}
      </p>
      ...
      <div className="grid gap-3 lg:h-[30rem] lg:grid-cols-[1.65fr_1fr] lg:gap-4">
        <article className="group relative h-[18rem] overflow-hidden rounded-[1.5rem] bg-black sm:h-[22rem] lg:h-auto lg:rounded-[1.75rem]">
          ...
          <div className="absolute inset-x-0 bottom-0 p-7 text-white">
            <p className="text-[11px] font-semibold uppercase tracking-[.18em] text-[var(--primary)]">
              {lead.eyebrow}
            </p>
        ...
        <div className="grid min-h-0 grid-flow-col auto-cols-[82%] gap-3 overflow-x-auto pb-2 sm:auto-cols-[47%] lg:grid-flow-row lg:grid-rows-3 lg:gap-4 lg:overflow-visible lg:pb-0">
          {rail.map((item) => (
            <article
              key={item.title}
              className="grid min-h-[8.5rem] overflow-hidden rounded-[1.25rem] border border-black/10 bg-card grid-cols-[42%_1fr] lg:min-h-0"
            >
              <Image item={item.image} className="h-full min-h-0 w-full" />
              <div className="flex flex-col justify-center p-3 lg:p-5">
                <p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[var(--brand-primary-strong)]">
                  {item.eyebrow}
                </p>
    ...
    <section>
      ...
      <div className="grid grid-flow-col auto-cols-[78%] gap-3 overflow-x-auto pb-2 sm:auto-cols-[46%] md:grid-flow-row md:grid-cols-3 md:overflow-visible md:pb-0 lg:gap-4">
        {items.map((item) => (
          <article
            key={item.title}
            className="group overflow-hidden rounded-[1.5rem] border border-black/10 bg-card shadow-sm"
          >
            <div className="relative h-48 overflow-hidden lg:h-56">
              ...
              <div className="absolute inset-x-0 bottom-0 p-5 text-white">
                <p className="text-[10px] uppercase tracking-[.16em] text-[var(--primary)]">
    ...
    <section>
      ...
      <div className="grid grid-flow-col auto-cols-[72%] gap-3 overflow-x-auto pb-2 sm:grid-flow-row sm:grid-cols-2 sm:overflow-visible sm:pb-0 lg:grid-cols-4 lg:gap-4">
        {cards.map((item) => (
          <article
            key={item.title}
            className="overflow-hidden rounded-[1.25rem] border border-black/10 bg-card"
          >
            <Image item={item.image} className="h-36 w-full lg:h-44" />
            <div className="p-4">
              <p className="text-[10px] uppercase tracking-[.15em] text-[var(--brand-primary-strong)]">
                {item.type}
              </p>
    ...
    <section>
      ...
      <div className="grid grid-flow-col auto-cols-[82%] overflow-x-auto rounded-[1.5rem] bg-[var(--selva)] text-[var(--selva-foreground)] sm:auto-cols-[48%] md:grid-flow-row md:grid-cols-3 md:overflow-hidden">
        {[
          ...
          >
            <div>
              <p className="font-serif text-4xl text-[var(--primary)]">{day}</p>
...
function TerritoryMap() {
  return (
    <section>
      ...
      />
      <div className="grid overflow-hidden rounded-[1.5rem] border border-black/10 bg-card lg:min-h-[27rem] lg:grid-cols-[1.65fr_1fr] lg:rounded-[1.75rem]">
        <div className="relative h-[15rem] bg-[var(--brand-surface-soft)] sm:h-[20rem] lg:h-auto">
          <div
            className="absolute inset-0 opacity-30"
            style={{
              backgroundImage: "radial-gradient(var(--selva) 1px, transparent 1px)",
              backgroundSize: "18px 18px",
            }}
          />
          <div className="absolute left-[44%] top-[47%] rounded-full bg-[var(--primary)] p-3 shadow-xl">
            <MapPin className="size-5 text-[var(--selva)]" />
          </div>
          <span className="absolute left-[39%] top-[62%] rounded-full bg-card px-3 py-1 text-xs font-semibold shadow">
            Valladolid
          </span>
        </div>
        <div className="divide-y divide-black/10 p-4 lg:p-6">
          {[
            ...
            <div key={place} className="flex items-center gap-4 py-5">
              <span className="grid size-10 place-items-center rounded-full bg-[var(--background)] text-[10px] font-semibold text-[var(--brand-primary-strong)] lg:size-12 lg:text-xs">
                {distance}
              </span>
              <div>
...
function TripClose() {
  return (
    <section className="grid items-center gap-4 rounded-[1.5rem] bg-[var(--selva)] px-5 py-5 text-[var(--selva-foreground)] sm:grid-cols-[auto_1fr_auto] lg:gap-5 lg:rounded-[1.75rem] lg:px-7 lg:py-7">
      ...
      <div>
        <p className="text-xs uppercase tracking-[.18em] text-[var(--primary)]">
