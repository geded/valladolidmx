 ...
 */
import type { EntityRef, InspectorViewDescriptor } from "@/lib/workspace/types";
import { findInspectorFor, listInspectorsFor } from "@/lib/workspace/context-registry";

const runtime = new Map<string, InspectorViewDescriptor[]>();

export function registerInspectorView(workspaceId: string, view: InspectorViewDescriptor) {
...
export function resolveInspector(
  ...
  const runtimeMatch = runtime.get(workspaceId)?.find((v) => v.entityType === entity.type);
  return runtimeMatch ?? findInspectorFor(workspaceId, entity.type);
}

export function listAllInspectors(workspaceId: string): InspectorViewDescriptor[] {
