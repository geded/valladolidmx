-- CV8.9.2 · Corrective hardening after PR review
--
-- Keeps the applied migration immutable while closing two concurrency gaps:
-- one decision may have only one direct replacement, and current state is
-- ordered by the database-authoritative occurred_at timestamp first.

CREATE UNIQUE INDEX IF NOT EXISTS visitor_intel_events_supersedes_decision_uidx
  ON visitor_intel.events (
    ((payload -> 'payload' ->> 'supersedes_decision_id'))
  )
  WHERE kind = 'recommendation.lifecycle'
    AND payload ->> 'subtype' = 'decision'
    AND payload -> 'payload' ->> 'supersedes_decision_id' IS NOT NULL;
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, visitor_intel
AS $$
DECLARE
  v_payload JSONB;
  v_event_id UUID;
...
BEGIN
  ...
  IF v_supersedes_decision_id IS NOT NULL AND EXISTS (
    SELECT 1
      FROM visitor_intel.events e
     WHERE e.kind = 'recommendation.lifecycle'
       AND e.payload ->> 'subtype' = 'decision'
       AND e.payload -> 'payload' ->> 'supersedes_decision_id' =
         v_supersedes_decision_id::TEXT
  ) THEN
    RAISE EXCEPTION 'decision has already been superseded' USING ERRCODE = '23514';
  END IF;
...
COMMENT ON FUNCTION visitor_intel.append_decision_event(JSONB, TEXT) IS
  'CV8.9.2 atomic append-only channel; corrective supersede and ordering hardening.';
