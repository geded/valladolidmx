...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'orders', jsonb_build_object(
      ...
      'paid',  (SELECT count(*) FROM public.orders WHERE status IN ('confirmed'::public.order_status,'fulfilled'::public.order_status))
    ),
    'revenue', jsonb_build_object(
      'gross_cents', COALESCE((SELECT sum(total_amount_cents) FROM public.orders
                                WHERE status IN ('confirmed'::public.order_status,'fulfilled'::public.order_status)), 0),
      'currency', 'MXN'
    ),
