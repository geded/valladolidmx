...
**Verdict:** SINGLE-ACTOR OPERATIONAL ACCEPTANCE: PASS

**Accessibility assisted:** DEFERRED — non-blocking until formal accessibility certification

**Multi-actor operation:** DEFERRED — mandatory before enabling a second editorial identity
**Open P0:** 0  
**Open P1:** 0
...
> registrados como **DEFERRED** con dependencia explícita: _segunda identidad editorial
> real y sesión operativa de aprobación_. No bloquean el avance funcional y visual de
...
> **Decisión Founder 2026-08-25 (CIERRE SINGLE-ACTOR):** Valladolid.mx opera
> actualmente con una sola identidad editorial. Se acepta operativamente I4-D para ese
> modelo sin simular cuentas ni fabricar evidencia. `G3-H07` permanece **DEFERRED —
> accesibilidad asistida no bloqueante** y deberá ejecutarse con lector real antes de una
> certificación formal de accesibilidad. `G3-H11` y la porción multi-actor de `G3-H12`
> permanecen **DEFERRED** y serán requisito fail-closed antes de habilitar una segunda
> identidad editorial. La separación técnica autor–aprobador no se elimina ni relaja.

## 1. Regla de cierre

Este reporte es la autoridad Founder de aceptación para G3. Para el modelo operativo
single-actor vigente, G3 puede cerrar cuando:

1. todos los escenarios actualmente ejecutables (`G3-H01`–`G3-H06` y
   `G3-H08`–`G3-H10`) estén en `PASS` humano u objetivo admisible;
...
`G3-H07`, `G3-H11` y la porción multi-actor de `G3-H12` no se declaran PASS: son
prerrequisitos explícitos para certificación formal o para habilitar una segunda
identidad editorial. `G3-H05` queda acreditado por evidencia objetiva admisible
...
| G3-H07 | Lector real: VoiceOver/Safari, NVDA o TalkBack según dispositivo disponible | DEFERRED — accesibilidad asistida no bloqueante                    | Requiere lector real antes de una certificación formal de accesibilidad. No se declara PASS ni se fabrica evidencia humana. La evidencia automatizada de teclado, foco y ausencia de trampa permanece en G3-H06.                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| G3-H11 | Slice vertical completo: authorable + binding + preview + Draft→Published + autor/aprobador distintos + conflicto + auditoría + rollback | DEFERRED — segunda identidad no habilitada | Requisito fail-closed antes de habilitar una segunda identidad editorial. No se simularon cuentas. Los contratos automatizados I4-B/C/R conservan separación autor–aprobador, conflicto, auditoría y rollback. |
| G3-H12 | Seguridad: operaciones sensibles con actor autorizado, preview sin PII/secrets y fallo gobernado fail-closed | DEFERRED — segunda identidad no habilitada | Requisito operativo antes de habilitar una segunda identidad. Los controles automatizados SEC-04/08/09 y el preview noindex/fail-closed permanecen PASS; no se declara validación humana multi-actor. |
...
- **DEF-G3-001 · Canvas del Experience Builder sin encabezado y con desbordamiento horizontal en vista previa Móvil.**
  **CLOSED (2026-08-22)** por validación humana repetida en iPad Safari sobre
  ...
  Estado de aceptación del gate: **G3 `PASS` para operación single-actor**. G3-H07
  (VoiceOver) queda diferido hasta certificación formal; G3-H11 y la porción multi-actor
  de G3-H12 serán bloqueantes antes de habilitar una segunda identidad. G3-H05 queda acreditado
...
**SINGLE-ACTOR OPERATIONAL ACCEPTANCE: PASS.** Es el modelo real vigente de
Valladolid.mx. No se crea sandbox, usuarios, permisos, migraciones ni RPC.

**DEFERRED NO BLOQUEANTE:** `G3-H07` se ejecutará con lector real antes de una
certificación formal de accesibilidad. `G3-H11` y la porción multi-actor de `G3-H12`
deberán completarse antes de habilitar una segunda identidad editorial.

**Efecto sobre la automatización.** `validate:i4:d` reconoce únicamente este modelo
single-actor y las tres deferencias nominales. Falla si cualquier escenario ejecutable
deja de estar en PASS, si una deferencia se oculta como PASS, si se elimina la obligación
previa a segunda identidad o si se debilita la separación técnica autor–aprobador.
