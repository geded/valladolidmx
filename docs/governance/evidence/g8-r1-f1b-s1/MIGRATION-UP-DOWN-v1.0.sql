-- G8-R1-F1B-S1 · MIGRACIÓN APLICADA (UP) + ROLLBACK (DOWN)
-- Fuente: supabase/migrations/20260829083223_dd4dc1c5-049a-451d-a308-b1c13f6027c4.sql
-- Aditiva, idempotente, reversible. Cero backfill, cero fichas, cero publicación.
...
-- ---- Migración complementaria 20260829083941 (ACL fail-closed) ----
...
GRANT ALL ON public.business_claim_snapshots TO service_role;
-- =========================================================================
-- ROLLBACK (DOWN) · reversible y idempotente
-- =========================================================================
DROP FUNCTION IF EXISTS public.business_public_source_summary(uuid);
DROP FUNCTION IF EXISTS public.resolve_business_claim_state(uuid);
DROP FUNCTION IF EXISTS public.create_business_claim_snapshot(uuid, text, uuid);

DROP TRIGGER IF EXISTS bcs_immutable_guard_trg ON public.business_claim_snapshots;
DROP TABLE IF EXISTS public.business_claim_snapshots;
DROP FUNCTION IF EXISTS public.bcs_immutable_guard();

ALTER TABLE public.business_hours
  DROP COLUMN IF EXISTS source_verified_at,
  DROP COLUMN IF EXISTS valid_until;

ALTER TABLE public.businesses
  DROP CONSTRAINT IF EXISTS businesses_record_origin_allowlist,
  DROP CONSTRAINT IF EXISTS businesses_source_review_state_allowlist,
  DROP COLUMN IF EXISTS record_origin,
  DROP COLUMN IF EXISTS source_review_state,
  DROP COLUMN IF EXISTS last_verified_at,
  DROP COLUMN IF EXISTS verification_due_at;

DROP TRIGGER IF EXISTS efp_history_guard_trg ON public.entity_field_provenance;
DROP TRIGGER IF EXISTS efp_assert_entity_exists_trg ON public.entity_field_provenance;
DROP TABLE IF EXISTS public.entity_field_provenance;
DROP FUNCTION IF EXISTS public.efp_can_read_row(uuid, text, uuid);
DROP FUNCTION IF EXISTS public.efp_history_guard();
DROP FUNCTION IF EXISTS public.efp_assert_entity_exists();
