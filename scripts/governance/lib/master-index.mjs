...
export function readMasterIndex(root = process.cwd()) {
  ...
  const rows = source
    ...
    .map((line) => {
      ...
      if (!documentPath || cells.length < 13)
        throw new Error(`Malformed 06 row: ${line.slice(0, 140)}`);
      return {
        path: documentPath,
        title: cells[1],
        version: value(2),
...
export function exactImplementationPaths(row) {
  if (!row.implementation || row.implementation === "—" || row.implementation === "Not established")
    return [];
  return row.implementation
    .split(";")
    .map((value) => value.trim())
    .filter((value) => value && !value.endsWith("/") && !value.includes("*"));
}
...
export function validateMasterIndex(index, root = process.cwd()) {
  ...
  for (const row of index.rows) {
    ...
    if (!ALLOWED_STATES.has(row.state))
      errors.push(`Unauthorized state in 06: ${row.path} (${row.state})`);
    if (!row.domainId) errors.push(`Missing canonical D01-D14 owner in 06: ${row.path}`);
    if (!row.authority || row.authority === "—")
      errors.push(`Missing admission evidence in 06: ${row.path}`);
    if (!fs.existsSync(path.join(root, row.path)))
      errors.push(`06 references missing Blueprint: ${row.path}`);
  }
  const files = fs
    .readdirSync(path.join(root, "docs/blueprint"), { recursive: true, withFileTypes: true })
    .filter((entry) => entry.isFile())
    .map((entry) =>
      path.posix.join(
        "docs/blueprint",
        entry.parentPath.replaceAll("\\", "/").split("docs/blueprint").pop().replace(/^\//, ""),
        entry.name,
      ),
    )
  ...
  for (const file of files)
    if (!seen.has(file)) errors.push(`Blueprint requires admission in 06: ${file}`);
  if (files.length !== index.rows.length)
    errors.push(`Blueprint universe mismatch: tree=${files.length}, 06=${index.rows.length}`);
  return errors;
}
