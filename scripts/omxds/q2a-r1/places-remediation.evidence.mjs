...
const NEW_COLUMNS = ["directions", "admission_kind", "price_to", "contact_whatsapp", "social_links", "published_at"];
const PLACE_TABLES = [
  "place_types",
  "place_categories",
...
for (const table of NEW_TABLES) expect(r1.includes(`CREATE TABLE IF NOT EXISTS public.${table}`), `falta la tabla ${table}`);
for (const column of NEW_COLUMNS) expect(r1.includes(`ADD COLUMN IF NOT EXISTS ${column}`), `falta la columna ${column}`);
for (const table of PLACE_TABLES)
  expect(r1.includes(`REVOKE ALL ON public.${table} FROM PUBLIC, anon, authenticated;`), `sin REVOKE mínimo en ${table}`);

expect(!/GRANT[^;]*ON public\.place_authorities TO[^;]*anon/.test(r1), "place_authorities expuesta a anon");
expect(!/ALTER DEFAULT PRIVILEGES/i.test(r1), "modifica default privileges globales");
expect(!/REVOKE[^;]*ON public\.points_of_interest/.test(r1), "modifica el ACL histórico de points_of_interest");
expect(!/CREATE\s+TYPE/i.test(r1), "introduce un enum de PostgreSQL");
expect(/place_duplicate_warnings[\s\S]*SECURITY INVOKER/.test(r1), "place_duplicate_warnings no es SECURITY INVOKER");
expect(!/place_duplicate_warnings[\s\S]*SECURITY DEFINER/.test(r1), "place_duplicate_warnings sigue siendo SECURITY DEFINER");
...
expect(!/Chich[eé]n|Ek.?\s?Balam|Tinum|Temoz[oó]n|Suytun|Zazil|Venados/i.test(r1), "menciona lugares reales excluidos");
expect(taxonomy.includes("PLACE_ADMISSION_KINDS"), "la taxonomía no declara los códigos de acceso");
expect(taxonomy.includes("PLACE_PRODUCT_RELATION_KINDS") && taxonomy.includes("PLACE_EVENT_RELATION_KINDS"), "faltan códigos de relación");
...
const report = {
  ...
  security_definer_retained: ["admin_create_place", "admin_update_place_details", "admin_set_place_categories"],
