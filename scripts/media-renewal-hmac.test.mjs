...
function verify(input) {
  ...
  if (Buffer.byteLength(input.body, "utf8") > RENEW_BODY_MAX_BYTES)
    return { ok: false, reason: "body_too_large" };
  if (!input.signatureHeader || !input.timestampHeader || !input.nonceHeader)
    return { ok: false, reason: "missing_header" };
  const nonce = input.nonceHeader.trim();
  if (nonce.length < 24 || nonce.length > 128 || !/^[a-zA-Z0-9._-]+$/.test(nonce))
    return { ok: false, reason: "missing_header" };
  ...
  const expected = hmacHex(
    input.secret,
    canonical("POST", input.path, String(ts), nonce, sha256Hex(input.body)),
  );
  if (input.signatureHeader.length !== expected.length)
    return { ok: false, reason: "bad_signature" };
  // Constant-time compare (via Buffer.compare — length equal above).
  const a = Buffer.from(input.signatureHeader, "hex"),
